...
@Description("Gets the prefix of a PermissionsEx group")
@Examples({"message \"%group \"\"admin\"\" prefix in world player's world%\"", 
			"set group \"admin\" prefix to \"[admin]\"",
			"delete group \"admin\" prefix"})
@Since("0.4.0")
...
public class ExprPrefix extends SimpleExpression<String> {
	...
	static {
		Skript.registerExpression(ExprPrefix.class, String.class, ExpressionType.SIMPLE,
				"[pex] group %string% prefix [in [world] %-world%]");
	}
	...
	public String toString(@Nullable Event arg0, boolean arg1) {
		return "[pex] group %string% prefix [in [world] %-world%]";
	}
	...
	protected String[] get(Event arg0) {
		...
		return new String[] { group.getPrefix(worldName) };
	}
	...
	public void change(Event e, Object[] delta, ChangeMode mode) {
		...
		if (mode == ChangeMode.SET) {
			...
			group.setPrefix(prefix, worldName);
		} else if (mode == ChangeMode.DELETE) {
			group.setPrefix(null, worldName);
